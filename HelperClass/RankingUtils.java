 ...
 * RankingUtils
 *
 * Utility class responsible for computing ranking scores for items
 * based on multiple user preferences and item attributes.
 *
 * This class implements a multi-feature ranking system used in the
 * Explore section of the application.
 *
 * Features considered during ranking:
 *
 * 1. Category match
 * 2. Distance from the user
 * 3. Keyword similarity
 * 4. Price compatibility with user's budget
 * 5. Item rating
 *
 * The class also includes a lightweight machine learning approach
 * using SGD (Stochastic Gradient Descent) to update ranking weights
 * based on user interaction history.
 *
 * Main responsibilities:
 *
 * - Distance calculation (Haversine formula)
 * - Feature vector generation for ranking
 * - Score calculation using linear weighting or sigmoid
 * - Weight optimization using user behavior
 *
 * This utility is used by the recommendation / explore system
 * to sort items based on relevance to the user.
 */
public class RankingUtils {
    /**
     * Pattern used to split text into tokens for keyword similarity.
     */
    private static final Pattern TOKEN_SPLIT = Pattern.compile("\\W+");
    /**
     * Calculates geographical distance between two coordinates
    ...
    /**
     * Converts distance into a normalized score between 0 and 1.
     * Closer items receive higher scores.
     */
    ...
    /**
     * Computes price compatibility score relative to user budget.
     */
    ...
    /**
     * Tokenizes text and converts it into a unique set of keywords.
     */
    ...
    /**
     * Calculates keyword similarity between the user's search query
     * and the item's description.
     */
    ...
    /**
     * Converts rating value (0–5) into normalized score (0–1).
     */
    ...
    /**
     * Builds the feature vector used by the ranking algorithm.
     *
     * Feature order:
     *
     * [category, distance, keyword, price, rating]
     */
    // Order: [category, distance, keyword, price, rating]
    public static double[] buildFeatureVector(
        ...
        try {
            if (userCategory != null && !userCategory.trim().isEmpty()) {
                if (item != null && item.getCategory() != null && item.getCategory().equalsIgnoreCase(userCategory.trim())) {
                    catScore = 1.0;
                } else {
                    catScore = 0.0;
                }
    ...
    /**
     * Calculates final ranking score using linear weighted sum.
     */
    ...
    // existing linear compute (weights array)
    ...
    // new overload: accept Map<String,Double> weightsMap
    public static double computeItemScore(
            ItemsModel item,
            String userCategory,
            String userQueryKeywords,
    ...
    /**
     * Sigmoid activation used for logistic prediction.
     */
    ...
    /**
     * Updates model weights using Stochastic Gradient Descent.
     *
     * Used to adapt ranking behavior based on
    ...
    /**
     * Ensures value remains within [0,1].
     */
    ...
    /* -------------------------
       New helpers for Map<->array and online update from history
       ------------------------- */

    // map keys: "category","distance","keyword","price","rating"
    ...
    /**
     * updateWeights:
     * - existing: current user-specific weight map (nullable)
     * - viewedItems: لیست آیتم‌هایی که کاربر دیده (مثبت)
     * الگوریتم: برای هر آیتم یک SGD step با label=1.0 انجام می‌دهیم تا وزن‌ها به سمت ویژگی‌های آیتم‌های دیده‌شده میل کنند.
     */
    public static Map<String, Double> updateWeights(Map<String, Double> existing, List<ItemsModel> viewedItems) {
        ...
        // محاسبه بودجه متوسط کاربر از آیتم‌های دیده‌شده (اگر ممکن باشد)
